...
public:
    ...
    // Fungsi hitung jarak
    void hitungJarak() {
        // [ORIGINAL] Jarak dari titik pusat (0, 0)
        // jarak_pusat = std::sqrt((x * x) + (y * y));

        // [MODIFIKASI] Jarak dari titik acuan baru (5.0, 5.0)
        double dx = x - 5.0;
        double dy = y - 5.0;
        jarak_pusat = std::sqrt((dx * dx) + (dy * dy));
    }

    // Getter: satu-satunya cara akses data privat dari luar kelas
